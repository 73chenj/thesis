...
ax.plot(
    cr3bp_states_capstone[:, 0],
    cr3bp_states_capstone[:, 1],
    cr3bp_states_capstone[:, 2],
    label="CAPSTONE",
)
...
ax.legend()
# plt.show()
fig.savefig("AWP/output/3D_Trajectory_of_CAPSTONE_and_ELFO.png")
...
ax1.plot(range(len(ranges)), ranges, "o")
ax1.set_title("Ranges")

ax2.plot(range(len(range_rates)), range_rates, "o")
ax2.set_title("Range Rates")

plt.tight_layout()
# plt.show()
fig.savefig("AWP/output/Ranges_and_Range_Rates.png")
...
prev_state_covariance_matrix = np.diag(
    [
        1 / d,
        1 / d,
        1 / d,
