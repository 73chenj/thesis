...
SURFACE_BODY_MAP = {"earth": EARTH_SURFACE_IMAGE, "jupiter": JUPITER_SURFACE_IMAGE}
...
def plot_reference_frames(frames, args, vectors=[], plots=[], planes=[]):
    ...
    if _args["base_frame"]:
        ...
        if _args["vector_texts"]:
            ax.text(_args["frame_text_scale"], 0, 0, "X", color=_args["base_color"])
            ax.text(0, _args["frame_text_scale"], 0, "Y", color=_args["base_color"])
            ax.text(0, 0, _args["frame_text_scale"], "Z", color=_args["base_color"])
    n = 0
    for plot in plots:
        ax.plot(
            plot[:, 0],
